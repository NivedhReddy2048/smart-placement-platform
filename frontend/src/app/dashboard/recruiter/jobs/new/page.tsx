...
import { Briefcase, MapPin, DollarSign, FileText, Plus, ArrowLeft } from 'lucide-react';
...
export default function NewJobPage() {
  ...
  useEffect(() => {
    const fetchRoles = async () => {
      try {
        const res = await apiClient.get('/core/job-roles/');
        setRoles(res.data);
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await apiClient.post('/jobs/postings/', {
      ...
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 p-6 md:p-10">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 p-8 sm:p-12 shadow-xl shadow-blue-500/5">
            <div className="mb-10">
              <h1 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight">Post a New Opportunity</h1>
              <p className="text-slate-500 dark:text-slate-400 font-medium mt-1">Fill in the details to attract the best talent.</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 py-4 bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-black rounded-2xl transition-all shadow-xl shadow-slate-200 dark:shadow-none hover:scale-[1.01] active:scale-95 disabled:opacity-50"
              >
